 ...
 * 
 * <p>
 * THis class is used by the SAX Parser to, event based, handle the content of
 * the XML file and the DTD file. This class needs a
 * <code>PlistProperties</code> object as input parameter to construct a data
 * tree.
 * </p>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Version 1.0: parsed almost everything except arrays or dictionaries that
 * were nested into arrays.
 ...
 * <li>Version 1.3: Dateformat corrected for <code>date</code> tags and
 * <code>real</code> tags can now also be understood by the handler and will be
 * stored as a <code>Double</code>.
 * </ul>
 * 
 * @author Gie Spaepen
 * @version 1.3
...
public class ReaderXMLHandler extends DefaultHandler implements EntityResolver {
	 ...
	 */
	private final PlistProperties rootObject;
	/**
	 ...
	 */
	private final Vector storageObjects = new Vector();

	/**
	 * This constructor needs a valid <code>PlistProperties</code> object. Don't
	 * use the the following code as constructor:<br>
	 * <code>ReaderXMLHandler reader = new ReaderXMLHandler(new PlistProperties());</code>
	 * <br>
	 * If you do so, there is no way to retrieve the data since the internal
	 * data storage is <code>private</code>
	 * 
	 * @param _properties
	 ...
	 * opening tags.<br>
	 * When a key, string, date, data or a integer tag passes by it sets the
	 * resp. flag to true in order that the <code>characters</code> function
	 ...
	 * @param _local
	 *            String - The tagname without prefixes (the one we need since
	 *            all the Plist tags are 'simple' tags.
	 * @param _raw
	 ...
	 */
	@Override
	public void startElement(String _URI, String _local, String _raw,
			Attributes atts) {
	 ...
	 * we only need to handle key, string, integer, data and data tags. A key
	 * tag is temporally stored in a string and later on reused to match it's
	 ...
	 * Generates an error if the date is malformed.
	 * 
	 * @param value
	 *            String
	 * @return Date
   ...
   * </code> function by
	 * 1) setting all the flags to false for a key, string, integer, date or
	 * data tag and moving a level down (say node) when encountering a
	 ...
	 * Since the PLIST files generally have a <code>DOCTYPE</code> tag with an
	 * URL in it the (stupid) SAX Parser want's to make an internet connection
	...
	public InputSource resolveEntity(String _pubId, String _sysId) {
		...
		} else if (_pubId.equals("-//Apple//DTD PLIST 1.0//EN")) {
			return new InputSource(getClass().getResourceAsStream("plist.dtd"));
		}
		//Pro forma
	 ...
	 * This can be an array, so only the value is added to a temporal
	 * <code>Vector</code> holding the array values, or to a
	 * <ocde>PlistProperties</code> object.
	 * 
	 * @param _key
	 *            String
	 * @param _value
	...
	private void setProperty(String _key, Object _value) {
		...
		Object currentObject = storageObjects
	 ...
	 * storage and when it is a dict a <code>PlistProperties</code> is added and
	 * all the data content is added to those objects. Last: use the parameter
	 * to the data storage. The added object will then act as current object to
	 * say whether it is an array or not...
	 * 
	 * @param _array
	 *            boolean - new level is an array level (true) or a dict level
	 *            (false)
	 */
	 ...
	 * to close the current object, say to fold the last node, and add them to
	 * the parent node. Determine if you're closing an array or not.
	 * 
	 * @param _array
		...
		if (storageObjects.size() > 1) {
			...
			if (_array) {
				try {
					...
					Vector currentObject = (Vector) storageObjects
							.elementAt(storageObjects.size() - 1);
					//...and its key.
					String currentObjectKey = (String) currentObject
							.elementAt(0);
