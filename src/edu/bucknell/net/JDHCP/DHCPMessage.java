...
public class DHCPMessage extends Object {
	...
	/**
	 * Creates empty DHCPMessage object, initializes the object, sets the host
	 * to a specified host name, and binds to a specified port.
	 * 
	 * @param inServername
	 *            the host name
	 * @param inPort
	...
	/**
	 * Creates empty DHCPMessage object, initializes the object, sets the host
	 * to a specified host name, and binds to the default port.
	 * 
	 * @param inServername
	 *            the host name
	 */
	...
	/**
	 * Creates empty DHCPMessage object, initializes the object, sets the host
	 * to the broadcast address, and binds to a specified port.
	 * 
	 * @param inPort
	 *            the port number
	 */
	...
	/**
	 * Creates empty DHCPMessage object, initializes the object with a specified
	 * byte array containing DHCP message information, sets the host to default
	 * host name, the local subnet, and bind to the default server port.
	 * 
	 * @param ibuff
	...
	/**
	 * Creates empty DHCPMessage object, initializes the object with a specified
	 * byte array containing DHCP message information, sets the host to
	 * specified host name, and binds to the specified port.
	 * 
	 * @param ibuff
	 *            [] the byte array to initialize DHCPMessage object
	 * @param inServername
	 ...
	 * Creates empty DHCPMessage object, initializes the object with a specified
	 * byte array containing DHCP message information, sets the host to
	 * broadcast address, and binds to the specified port.
	 * 
	 * @param ibuff
	 *            [] the byte array to initialize DHCPMessage object
	 * @param inPort
	...
	/**
	 * DHCPMessage::Initialize initializes datamembers in the constructors every
	 * empty DHCPMessage object will by default contain these params.
	...
	public synchronized DHCPMessage internalize(byte[] ibuff) {
		ByteArrayInputStream inBStream = new ByteArrayInputStream(ibuff, 0,
				ibuff.length);
	...
	/**
	 * Get seconds elapsed since client began address acquisition or renewal
	 * process.
	 */
	...
	public void setOption(byte inOptNum, byte[] inOptionData) {
		optionsList.setOption(inOptNum, inOptionData);
	}
	 ...
	 *            option number
	 */
	public byte[] getOption(byte inOptNum) {
		if (optionsList == null) {
			Initialize();
		}
		return optionsList.getOption(inOptNum);
	}
	 ...
	 *            option number
	 */
	public void removeOption(byte inOptNum) {
		if (optionsList == null) {
			Initialize();
		}
		optionsList.removeOption(inOptNum);
	}
	 ...
	 * returned.
	 */
	public boolean IsOptSet(byte inOptNum) {
		if (optionsList == null) {
			Initialize();
		}
		return optionsList.contains(inOptNum);
	}

	public static String ipaddrToString(byte[] addr) {
		if (addr.length == 4) {
			return (0xFF & addr[0]) + "." + (0xFF & addr[1]) + "."
	...
	public String toString() {
		return getClass().getSimpleName() + "@" + hashCode() + "[op=" + op
				+ ",htype=" + htype + ",hlen=" + hlen + ",hops=" + hops
				+ ",xid=" + xid + ",secs=" + secs + ",flags=" + flags
				+ ",ciaddr=" + ipaddrToString(ciaddr) + ",yiaddr="
				+ ipaddrToString(yiaddr) + ",siaadr=" + ipaddrToString(siaddr)
				+ ",giaddr=" + ipaddrToString(giaddr) + ",chaddr="
				+ ipaddrToString(chaddr) + ",sname=" + new String(sname)
				+ ",file=" + new String(file) + "," + optionsList.toString()
				+ "]";
	}
}
